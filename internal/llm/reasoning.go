...
const ReasoningCommandDescription = "Set reasoning effort: /reasoning:<level> (default, minimal, low, medium, high)"
...
func ReasoningHelp(effort ReasoningEffort) string {
	...
	return fmt.Sprintf("reasoning effort: %s; choose with /reasoning:<level> (default, minimal, low, medium, high)", label)
}
...
func (s *ReasoningSelection) Set(effort ReasoningEffort) error {
	if effort != "" && effort != ReasoningEffortMinimal && effort != ReasoningEffortLow && effort != ReasoningEffortMedium && effort != ReasoningEffortHigh {
		return fmt.Errorf("unsupported reasoning effort %q (supported: default, minimal, low, medium, high)", effort)
	}
	s.mu.Lock()
	s.effort = effort
	s.mu.Unlock()
