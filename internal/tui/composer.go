...
func (c composer) handleKey(msg tea.KeyMsg, commands []command.Command, listFiles func() ([]string, error), models modelSource) (composer, composerIntent, tea.Cmd) {
	...
	if c.menuOpen() {
		...
		case tea.KeyEnter:
			...
			if selected.builtin && (selected.label == "/new" || selected.label == "/compact" || selected.label == "/resume" || selected.label == "/model" || selected.label == "/mcp" || selected.label == "/connect" || selected.label == "/mode" || selected.label == "/mode:auto-accept" || selected.label == "/mode:ask" || isDevelopmentBuiltinSelection(selected.label)) {
