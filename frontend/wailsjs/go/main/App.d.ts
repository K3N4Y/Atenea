// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT
import {main} from '../models';
import {learning} from '../models';
import {mcpclient} from '../models';
import {workspacegit} from '../models';
import {command} from '../models';
import {session} from '../models';

export function AcceptPlan(arg1:string):Promise<void>;

export function ActiveProvider():Promise<main.ActiveProvider>;

export function ApproveLearning(arg1:string,arg2:learning.Candidate):Promise<learning.Lesson>;

export function ApprovedLessons():Promise<Array<learning.Lesson>>;

export function AuditLearning():Promise<Array<learning.Run>>;

export function AwaitProviderLogin(arg1:string):Promise<void>;

export function CancelLearning(arg1:string):Promise<void>;

export function CancelProviderLogin(arg1:string):Promise<void>;

export function ClosePty(arg1:string):Promise<void>;

export function Commit(arg1:string):Promise<void>;

export function ConnectMCP(arg1:mcpclient.ServerConfig):Promise<mcpclient.ServerStatus>;

export function ConnectProvider(arg1:string,arg2:string):Promise<void>;

export function DeclareEndpoint(arg1:string,arg2:string,arg3:string):Promise<string>;

export function DeleteLesson(arg1:string):Promise<void>;

export function DeleteSession(arg1:string):Promise<void>;

export function DisconnectMCP(arg1:string):Promise<void>;

export function FileDiff(arg1:string):Promise<string>;

export function ForgetProvider(arg1:string):Promise<void>;

export function GenerateCommitMessage():Promise<string>;

export function GitStatus():Promise<workspacegit.Status>;

export function InitRepo():Promise<void>;

export function ListCommands():Promise<Array<command.Command>>;

export function ListMCPs():Promise<Array<mcpclient.ServerStatus>>;

export function ListModels(arg1:string):Promise<Array<string>>;

export function ListProjectFiles():Promise<Array<string>>;

export function ListSessions():Promise<Array<session.SessionSummary>>;

export function OpenLoginPage(arg1:string):Promise<void>;

export function PermissionMode(arg1:string,arg2:string):Promise<string>;

export function ProviderCatalog():Promise<Array<main.ProviderEntry>>;

export function QueueLearning(arg1:string):Promise<learning.Run>;

export function ReasoningEffort():Promise<string>;

export function RefreshModels():Promise<Array<main.ProviderEntry>>;

export function RejectLearning(arg1:string):Promise<void>;

export function RemoveMCPConfig(arg1:string):Promise<void>;

export function ResizePty(arg1:string,arg2:number,arg3:number):Promise<void>;

export function ResolveToolPermission(arg1:string,arg2:string,arg3:boolean):Promise<void>;

export function RetryLearning(arg1:string):Promise<learning.Run>;

export function SaveMCPConfig(arg1:mcpclient.ServerConfig):Promise<void>;

export function SelectModel(arg1:string,arg2:string):Promise<void>;

export function SelectWorkspace():Promise<string>;

export function SendPlanPrompt(arg1:string,arg2:string):Promise<void>;

export function SendPrompt(arg1:string,arg2:string):Promise<void>;

export function SessionHistory(arg1:string):Promise<Array<session.SessionEvent>>;

export function SetLessonEnabled(arg1:string,arg2:boolean):Promise<void>;

export function SetReasoningEffort(arg1:string):Promise<void>;

export function SetWorkspace(arg1:string):Promise<void>;

export function StartProviderLogin(arg1:string):Promise<main.DeviceLogin>;

export function StartPty(arg1:string,arg2:number,arg3:number):Promise<void>;

export function Stop(arg1:string):Promise<void>;

export function Workspace():Promise<string>;

export function WritePty(arg1:string,arg2:string):Promise<void>;
