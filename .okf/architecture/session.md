...
interface contains the `Store`, `CompactionStore`, and `UndoStore` persistence
contracts, and the input contracts used by the runner.
`internal/session/runner` consumes those contracts; Wails, TUI, and event
packages adapt them to their respective interfaces.

The **event model itself is published**: `SessionEvent`, `EventKind` and their
payloads live in `agentcore/session` and are re-exported here by `contract.go`,
...
- `compaction.go` and `undo.go` own their validation and projections, over the
  payload types published in `agentcore/session`;
...
Ask-before-run lives in its own module. The `Policy` (Allow/Ask/Deny verdict per
tool call) and the `Gate` that correlates a session tool call with a user
decision are published contracts in `agentcore/permission`;
`internal/permission` owns the classification atenea ships, the session grants
and the in-memory gate broker. The gate is not part of `Store`:
