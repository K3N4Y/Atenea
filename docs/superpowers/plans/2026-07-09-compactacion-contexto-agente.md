...
**Goal:** Implementar compactacion de contexto durable, preventiva al 80% y reactiva ante overflow, conservando el ultimo prompt del usuario, grupos completos de tools y una tarjeta auditable en la UI.

**Architecture:** El log de eventos permanece inmutable. Un compactor real selecciona historial, genera un resumen JSON con el mismo provider/modelo y confirma atomically un `Context.Compacted` junto con `ContextEpoch.BaselineSeq`. El runner reconstruye el request desde un `RunnerContext` durable; el frontend sigue rehidratando el log completo y representa el checkpoint como un item no conversacional.

**Tech Stack:** Go 1.23+, SQLite (`modernc.org/sqlite`), Wails v2, Vue 3, Pinia, TypeScript, Vitest.

---

## Precondiciones y disciplina TDD

- Ejecutar el plan desde un worktree dedicado. Si se crea una rama, usar prefijo `posthog-code/`.
- Antes de cada tarea, correr el test especifico de safety net indicado.
- Seguir por tarea: `Safety net -> Understand -> RED -> GREEN -> TRIANGULATE -> REFACTOR -> Evidence`.
- No avanzar si el test RED falla por compilacion ajena al comportamiento esperado.
- En codigo concurrente o atomico, repetir el gate con `-race` cuando el paquete lo permita.
- Cada commit usa estos trailers, despues de una linea vacia:
...
- `internal/llm/context.go`: metadatos de ventana, estimador conservador y error normalizado de overflow.
- `internal/llm/context_test.go`: limites, umbral y estimacion de system/tools/messages.
- `internal/session/compaction.go`: tipos `StructuredSummary`, `CompactionCheckpoint`, `RunnerContext` y errores de dominio.
- `internal/session/compaction_store_contract_test.go`: contrato compartido de checkpoint para MemoryStore y SQLiteStore.
- `internal/session/runner/compaction_select.go`: agrupacion semantica y seleccion de historial.
- `internal/session/runner/compaction_select_test.go`: ultimo usuario, tools y fallback por presupuesto.
- `internal/session/runner/compaction_summary.go`: llamada aislada, prompt JSON y validacion del resumen.
- `internal/session/runner/compaction_summary_test.go`: stream, timeout y respuestas invalidas.
- `internal/session/runner/compactor.go`: politica preventiva, armado del checkpoint y commit.
- `internal/session/runner/compactor_test.go`: pipeline real y compactaciones sucesivas.

### Nuevos archivos frontend

- `frontend/src/components/ContextCompactedCard.vue`: tarjeta expandible no conversacional.
- `frontend/src/components/ContextCompactedCard.test.ts`: render, toggle y campos del resumen.

### Archivos modificados

- `internal/llm/provider.go`: causa tipada en `llm.Event`.
- `internal/llm/openai.go`: clasificacion de overflow del stream OpenAI-compatible.
- `internal/llm/openai_test.go`: overflow vs error generico.
- `internal/session/session.go`: payload `Compaction` en `SessionEvent`.
- `internal/session/event.go`: `KindContextCompacted`.
- `internal/session/store.go`: `RunnerContext` y `CommitCompaction`.
- `internal/session/memstore.go`: epoch/checkpoint atomico en memoria.
- `internal/session/sqlitestore.go`: tabla `session_context`, payload JSON y transaccion CAS.
- `internal/event/store.go`: delegacion y emision de `CommitCompaction`.
- `internal/event/child_permission_store.go`: delegacion del nuevo contrato.
- `internal/session/runner/runner.go`: compactor real configurable.
- `internal/session/runner/turn.go`: proyeccion compactada y retry acotado.
- `internal/session/runner/turn_signals_test.go`: reemplazo del fake antiguo por senales nuevas.
- `internal/session/runner/turn_failure_test.go`: overflow reactivo y segundo overflow.
- `internal/wiring/wiring.go`: instalacion del compactor real.
- `internal/wiring/wiring_test.go`: wiring con y sin modelo conocido.
- `internal/tui/engine.go`: propagar el modelo resuelto al runner compartido.
- `internal/tui/engine_test.go`: request TUI usa la etiqueta de modelo configurada.
- `cmd/atenea-tui/main.go`: entregar a `EngineConfig` el modelo resuelto.
- `frontend/src/stores/chat.ts`: tipos y fold de `Context.Compacted`.
- `frontend/src/stores/chat.test.ts`: proyeccion live y rehidratada.
- `frontend/src/components/MessageList.vue`: registro del nuevo item.
- `frontend/src/components/MessageList.test.ts`: despacho a la tarjeta.
- `frontend/src/lib/contextWindow.ts`: alinear el catalogo visual con el backend.
- `frontend/src/lib/contextWindow.test.ts`: catalogo conocido sin cambiar el fallback visual actual.
...
### Task 8: Real compactor pipeline and runner-context projection
...
- Modify: `docs/atenea-agent-loop.md`
- Modify: `docs/atenea-agent-loop-roadmap.md`
...
Update `docs/atenea-agent-loop.md` so the compaction section states:
...
Mark the real M7/M10 compactor work complete in `docs/atenea-agent-loop-roadmap.md` without claiming unrelated roadmap items are complete.
...
7. A forced normalized overflow retries once; a second overflow shows one clean error and no infinite spinner.
...
```bash
git add frontend/src/lib/contextWindow.ts frontend/src/lib/contextWindow.test.ts docs/atenea-agent-loop.md docs/atenea-agent-loop-roadmap.md
git commit -m "$(cat <<'EOF'
...
| Understand | Spec, runner, store, provider, event decorators and UI fold read | `docs/superpowers/specs/2026-07-09-compactacion-contexto-agente-design.md` plus mapped files | behavior identified |
| RED | Each task's behavior test failed first | exact `go test -run ... -v` / `vitest` commands | expected behavioral failure, gate ok |
...
- [ ] Every checkpoint is durable and reconstructible after restart.
