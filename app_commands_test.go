...
func TestApp_ListCommandsReturnsRegisteredCommands(t *testing.T) {
	...
	if len(cmds) != 8 || cmds[0].Name != "abc" || cmds[1].Name != "commit" || cmds[2].Name != "learn" || cmds[3].Name != "learned" || cmds[4].Name != "mode" || cmds[5].Name != "mode:auto-accept" || cmds[6].Name != "mode:ask" || cmds[7].Name != "variants" {
		t.Fatalf("ListCommands = %+v, want [abc commit learn learned mode mode:auto-accept mode:ask variants]", cmds)
	}
	if got := cmds[7].Description; got != llm.ReasoningCommandDescription {
		t.Fatalf("variants description = %q, want %q", got, llm.ReasoningCommandDescription)
	}
}
